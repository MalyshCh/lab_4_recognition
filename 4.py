...
class ImageProcessor:
    def __init__(self, root):
        ...
        # Create buttons
        self.load_button = Button(self.button_frame, text="Загрузить изображение", command=self.load_image)
        self.load_button.pack(side="left", padx=5)

        self.process_button = Button(self.button_frame, text="Обработать изображение", command=self.process_image)
        self.process_button.pack(side="left", padx=5)

        self.contour_button = Button(self.button_frame, text="Найти контур", command=self.find_contours)
        self.contour_button.pack(side="left", padx=5)

        self.primitive_button = Button(self.button_frame, text="Найти примитивы", command=self.find_primitives)
            ...
            for contour in contours:
                ...
                if cv2.contourArea(approx_contour) > min_area:
                    if len(approx_contour) == 3:
                        # Если контур содержит 3 точки, это треугольник
                        ...
                        # Если контур содержит 4 точки и углы прямые, это четырехугольник
    ...
    def is_rectangle(self, contour):
        ...
        for i in range(4):
            ...
            angles.append(angle)
        # Проверка, что все углы около 90 градусов
